...
echo [2/6] 编译顶层模块...
%MODELSIM_PATH%\vlog.exe -work work +incdir+%RTL_DIR% tdc_top_carry4.v
if errorlevel 1 (
...
echo [3/6] 编译核心模块...
%MODELSIM_PATH%\vlog.exe -work work +incdir+%RTL_DIR% carry4_delay_chain.v
%MODELSIM_PATH%\vlog.exe -work work +incdir+%RTL_DIR% coarse_counter_400m.v
%MODELSIM_PATH%\vlog.exe -work work +incdir+%RTL_DIR% edge_detector_sync.v
%MODELSIM_PATH%\vlog.exe -work work +incdir+%RTL_DIR% thermometer_decoder.v
%MODELSIM_PATH%\vlog.exe -work work +incdir+%RTL_DIR% timestamp_synthesizer.v
%MODELSIM_PATH%\vlog.exe -work work +incdir+%RTL_DIR% mmcm_50m_to_400m.v
if errorlevel 1 (
...
echo [4/6] 编译测试平台...
%MODELSIM_PATH%\vlog.exe -work work +incdir+%RTL_DIR% tb_carry4_tdc.v
if errorlevel 1 (
